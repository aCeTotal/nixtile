...
struct Monitor {
	...
	float workspace_pending_target_mfact[9]; /* Per-workspace pending mfact */
	float workspace_pending_target_height_factor[9]; /* Per-workspace pending height factor */
	int workspace_pending_left_column[9]; /* Per-workspace pending left column */
	int workspace_pending_right_column[9]; /* Per-workspace pending right column */
	int workspace_pending_left_width[9]; /* Per-workspace pending left width */
	int workspace_pending_right_width[9]; /* Per-workspace pending right width */
	int workspace_pending_right_x[9]; /* Per-workspace pending right x position */
...
static void load_workspace_state(void);
...
rebalance_column_assignments(Monitor *m)
{
	...
	/* WORKSPACE ISOLATION: Count total tiles that need column assignment (workspace-specific) */
	int total_tiles = 0;
	Client *c;
	wl_list_for_each(c, &clients, link) {
		/* WORKSPACE ISOLATION: Only count tiles visible in current workspace */
		if (VISIBLEON(c, m) && !c->isfloating && !c->isfullscreen) {
			total_tiles++;
		}
	}
	...
	/* WORKSPACE ISOLATION: First pass - count existing valid assignments to preserve distribution (workspace-specific) */
	wl_list_for_each(c, &clients, link) {
		/* WORKSPACE ISOLATION: Only process tiles visible in current workspace */
		if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
			continue;
	...
	if (has_empty_columns && total_tiles > 0) {
			...
			int tile_index = 0;
			wl_list_for_each(c, &clients, link) {
				/* WORKSPACE ISOLATION: Only redistribute tiles visible in current workspace */
				if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
					continue;
			...
			/* WORKSPACE ISOLATION: Recalculate column counts after redistribution (workspace-specific) */
	...
	/* WORKSPACE ISOLATION: Second pass - only reassign tiles with invalid column assignments (workspace-specific) */
	wl_list_for_each(c, &clients, link) {
		/* WORKSPACE ISOLATION: Only process tiles visible in current workspace */
		if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
			continue;
...
arrange_immediate(Monitor *m)
{
	...
	if (m->lt[m->sellt]->arrange) {
		/* WORKSPACE INDEPENDENCE: Ensure workspace state is loaded before layout */
		load_workspace_state();
		
		/* PREVENT RESIZE SNAP-BACK: Skip layout during active tile resize to prevent interference */
		if (cursor_mode == CurTileResize || resize_operation_active || vertical_resize_pending || resize_pending) {
			wlr_log(WLR_DEBUG, "[nixtile] ARRANGE: Skipping layout recalculation during active resize operation (cursor_mode=%d, resize_active=%d, v_pending=%d, h_pending=%d)", 
			        cursor_mode, resize_operation_active, vertical_resize_pending, resize_pending);
			return;
		}
		...
		wl_list_for_each(temp_c, &clients, link) {
			if (VISIBLEON(temp_c, m) && !temp_c->isfullscreen) {
				...
				int current_optimal_columns = get_workspace_optimal_columns();
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	/* ULTRA-AGGRESSIVE DEBUG: Force immediate log flush */
	wlr_log(WLR_ERROR, "[nixtile] *** BUTTONPRESS ENTRY *** button=%d, state=%d, time=%u", event->button, event->state, event->time_msec);
	fflush(stderr); /* Force immediate output */
	wlr_log(WLR_ERROR, "[nixtile] *** BUTTON EVENT RECEIVED - PROCESSING ***");
	wlr_log(WLR_ERROR, "[nixtile] *** BUTTON VALUES: BTN_LEFT=0x%x, BTN_MIDDLE=0x%x, BTN_RIGHT=0x%x ***", BTN_LEFT, BTN_MIDDLE, BTN_RIGHT);
	wlr_idle_notifier_v1_notify_activity(idle_notifier, seat);
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		wlr_log(WLR_ERROR, "[nixtile] *** BUTTON PRESS: button=%d, mods=0x%x ***", event->button, mods);
		for (b = buttons; b < END(buttons); b++) {
			wlr_log(WLR_ERROR, "[nixtile] BUTTON CHECK: checking button=%d mod=0x%x vs event button=%d mods=0x%x", 
				b->button, b->mod, event->button, CLEANMASK(mods));
			if (CLEANMASK(mods) == CLEANMASK(b->mod) &&
					event->button == b->button && b->func) {
				wlr_log(WLR_ERROR, "[nixtile] *** BUTTON MATCH FOUND - CALLING FUNCTION ***");
				b->func(&b->arg);
				return;
			}
		}
		wlr_log(WLR_ERROR, "[nixtile] *** NO BUTTON MATCH FOUND ***");
		break;
...
createmon(struct wl_listener *listener, void *data)
{
	...
	/* Initialize all workspace states to uninitialized */
	for (int ws = 0; ws < 9; ws++) {
		m->workspace_initialized[ws] = false;
		...
		for (int i = 0; i < MAX_TILES_PER_WORKSPACE; i++) {
			m->workspace_height_factors[ws][i] = 1.0f;
		}
		
		/* WORKSPACE ISOLATION: Initialize pending state for layout isolation */
		m->workspace_pending_target_mfact[ws] = 0.5f;
		m->workspace_pending_target_height_factor[ws] = 1.0f;
		m->workspace_pending_left_column[ws] = -1;
		m->workspace_pending_right_column[ws] = -1;
		m->workspace_pending_left_width[ws] = 0;
		m->workspace_pending_right_width[ws] = 0;
		m->workspace_pending_right_x[ws] = 0;
	}
...
createnotify(struct wl_listener *listener, void *data)
{
	...
	/* Check if all columns are at maximum capacity */
	bool all_columns_full = true;
	for (int col = 0; col < optimal_columns && col < MAX_COLUMNS; col++) {
		if (column_counts[col] < MAX_TILES_PER_COLUMN) {
			all_columns_full = false;
			break;
		}
	}
	
	/* BLOCK CREATION: If all columns are full, prevent tile creation */
	if (all_columns_full) {
		wlr_log(WLR_ERROR, "[nixtile] *** CAPACITY LIMIT: All columns at max capacity (%d tiles each) - BLOCKING tile creation ***", MAX_TILES_PER_COLUMN);
		wlr_log(WLR_ERROR, "[nixtile] *** WORKSPACE FULL: Cannot create more tiles in current workspace ***");
		
		/* SAFE BLOCKING: Mark client as invalid and let normal cleanup handle it */
		c->tags = 0; /* Mark as invalid - will be cleaned up by normal mechanisms */
		c->isfloating = true; /* Prevent tiling operations */
		c->column_group = -1; /* Invalid column */
		
		/* Hide the client immediately to prevent visual artifacts */
		if (c->scene && c->scene->node.data) {
			wlr_scene_node_set_enabled(&c->scene->node, false);
		}
		
		wlr_log(WLR_DEBUG, "[nixtile] SAFE BLOCKING: Client marked for cleanup, exiting createnotify");
		return; /* Exit early - let normal cleanup handle the rest */
	}
	...
	/* STEP 2: Workspace overflow handling removed - tiles are blocked at capacity instead */
	/* Note: Capacity check above ensures we never exceed 5 tiles per column */
	
	/* STEP 3: SIMPLE GUARANTEED HORIZONTAL PLACEMENT */
	
	/* WORKSPACE ISOLATION: Count existing tiles in THIS workspace only (excluding the new tile) */
	int workspace_tile_number = current_workspace_tiles + 1; /* 1-based tile number for THIS workspace */
	
	wlr_log(WLR_INFO, "[nixtile] WORKSPACE ISOLATION: Workspace tile #%d (total in workspace: %d), master_tiles=%d, optimal_columns=%d", 
		workspace_tile_number, current_workspace_tiles, master_tiles, optimal_columns);
	...
	if (workspace_tile_number <= master_tiles) {
		...
		wlr_log(WLR_INFO, "[nixtile] WORKSPACE ISOLATION: Workspace tile #%d -> Column %d (guaranteed horizontal)", 
			workspace_tile_number, target_column);
		c->column_group = target_column;
	} else {
		/* STACKING: Let tile() function handle assignment for proper distribution */
		wlr_log(WLR_INFO, "[nixtile] WORKSPACE ISOLATION: Workspace tile #%d -> will be assigned by tile() function", 
...
handletiledrop(Client *c, double x, double y)
{
	...
	if (c->column_group == target_column) {
		...
		if (tiles_in_target_column == 0) {
			...
			/* SINGLE TILE to STACK: Check if it's a true swap or addition */
			if (prefer_intra_column && target_tile) {
				...
				/* TRUE SWAP: Single tile swaps with entire stack - ALWAYS ALLOWED even if stack has 5 tiles */
				wlr_log(WLR_DEBUG, "[nixtile] TRUE SWAP: Swapping single tile with entire stack in column %d (tiles=%d)", target_column, tiles_in_target_column);
			...
			/* TILE FROM STACK to SINGLE TILE: Check capacity before creating new stack */
			if (tiles_in_target_column + 1 > MAX_TILES_PER_COLUMN) {
				wlr_log(WLR_ERROR, "[nixtile] *** CAPACITY LIMIT: Cannot create stack in column %d (would exceed %d tiles) ***", target_column, MAX_TILES_PER_COLUMN);
				return;
			}
			...
			if (tiles_in_target_column >= MAX_TILES_PER_COLUMN) {
				wlr_log(WLR_ERROR, "[nixtile] *** CAPACITY LIMIT: Cannot add stacking tile to full column %d (%d/%d tiles) ***", target_column, tiles_in_target_column, MAX_TILES_PER_COLUMN);
				wlr_log(WLR_ERROR, "[nixtile] *** ADDITION BLOCKED: Stacking tile cannot join full stacking column ***");
				return;
			}
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (cursor_mode == CurMove) {
		...
		/* PREVENT INFINITE RESIZE LOOP: Use call depth counter instead of static flag */
		static int resize_call_depth = 0;
		if (resize_call_depth > 0) {
			wlr_log(WLR_DEBUG, "[nixtile] RESIZE LOOP PREVENTION: Skipping recursive resize call (depth=%d)", resize_call_depth);
			return;
		}
		resize_call_depth++;
		
		/* FORGIVING SAFETY CHECKS: Only abort on critical failures to prevent resize snap-back */
		if (!selmon || !grabc) {
			wlr_log(WLR_ERROR, "[nixtile] CRITICAL SAFETY: selmon or grabc is NULL in CurTileResize - ABORT");
			cursor_mode = CurNormal;
			resize_call_depth--;
			return;
		}
		
		/* FORGIVING: Only check for truly invalid geometry to prevent snap-back */
		if (!grabc->mon) {
			wlr_log(WLR_ERROR, "[nixtile] CRITICAL SAFETY: grabc has no monitor in CurTileResize - ABORT");
			cursor_mode = CurNormal;
			grabc = NULL;
			resize_call_depth--;
			return;
		...
		/* FORGIVING: Allow wider cursor range to prevent accidental resize termination */
		if (cursor->x < -50000 || cursor->x > 50000 || cursor->y < -50000 || cursor->y > 50000) {
			wlr_log(WLR_ERROR, "[nixtile] CRITICAL SAFETY: cursor position extremely out of bounds - ABORT");
			cursor_mode = CurNormal;
			resize_call_depth--;
			return;
		}
		
		/* Calculate mouse movement for bi-axial resizing */
		float horizontal_delta = cursor->x - (float)grabcx;
		float vertical_delta = cursor->y - (float)grabcy;
		
		/* BI-AXIAL RESIZE LOGIC: Mouse movement direction determines resize behavior */
		float abs_horizontal = fabs(horizontal_delta);
		float abs_vertical = fabs(vertical_delta);
		
		/* Determine resize direction based on movement magnitude and available edges */
		bool do_horizontal_resize = false;
		...
		if (abs_horizontal > 1.0f && abs_vertical > 1.0f) {
			...
			do_horizontal_resize = has_horizontal_edges;
			do_vertical_resize = has_vertical_edges;
			wlr_log(WLR_DEBUG, "[nixtile] BI-AXIAL RESIZE: Diagonal movement - h=%.1f, v=%.1f, do_h=%d, do_v=%d", 
				horizontal_delta, vertical_delta, do_horizontal_resize, do_vertical_resize);
		} else if (abs_horizontal > abs_vertical && abs_horizontal > 1.0f) {
			/* HORIZONTAL MOVEMENT: Primarily horizontal resizing */
			do_horizontal_resize = has_horizontal_edges;
			wlr_log(WLR_DEBUG, "[nixtile] HORIZONTAL RESIZE: h=%.1f > v=%.1f, do_h=%d", 
				horizontal_delta, vertical_delta, do_horizontal_resize);
		} else if (abs_vertical > abs_horizontal && abs_vertical > 1.0f) {
			/* VERTICAL MOVEMENT: Primarily vertical resizing */
			do_vertical_resize = has_vertical_edges;
			wlr_log(WLR_DEBUG, "[nixtile] VERTICAL RESIZE: v=%.1f > h=%.1f, do_v=%d", 
				vertical_delta, horizontal_delta, do_vertical_resize);
		}
		...
		if (do_horizontal_resize && horizontal_resize_client && horizontal_resize_neighbor) {
			...
			if (neighbor_tile) {
				...
				if (resizing_from_left_edge) {
					...
					/* Store ABSOLUTE right position - this must NEVER change */
					int absolute_right_x = grabc->geom.x + grabc->geom.width;
					
					/* SIMPLE RULE: 150px minimum width for ALL tiles - NO EXCEPTIONS */
					int current_width = grabc->geom.width;
					int neighbor_width = neighbor_tile->geom.width;
					
					/* Calculate what widths would be after resize */
					int new_current_width = absolute_right_x - (int)new_edge_x;
					int new_neighbor_width = ((int)new_edge_x - gap) - neighbor_tile->geom.x;
					
					/* STICKY MINIMUM: Enforce 150px minimum with stable clamping */
					if (new_current_width < 150 || new_neighbor_width < 150) {
						/* Calculate strict boundary positions */
						int max_edge_from_current = absolute_right_x - 150;  /* Current tile limit */
						int max_edge_from_neighbor = neighbor_tile->geom.x + 150 + gap;  /* Neighbor tile limit */
						
						/* STICKY LOGIC: Once at minimum, stay there until user moves away from boundary */
						int clamped_edge_x;
						if (new_current_width < 150) {
							/* Current tile hitting minimum - stick to boundary */
							clamped_edge_x = max_edge_from_current;
							wlr_log(WLR_DEBUG, "[nixtile] HORIZONTAL STICKY MINIMUM: Current tile at minimum, clamping to %d", clamped_edge_x);
						} else if (new_neighbor_width < 150) {
							/* Neighbor tile hitting minimum - stick to boundary */
							clamped_edge_x = max_edge_from_neighbor;
							wlr_log(WLR_DEBUG, "[nixtile] HORIZONTAL STICKY MINIMUM: Neighbor tile at minimum, clamping to %d", clamped_edge_x);
						} else {
							/* Should not reach here, but use safer boundary */
							clamped_edge_x = (max_edge_from_current + max_edge_from_neighbor) / 2;
						}
						
						/* FORCE: Set edge to clamped position - no further movement allowed */
						new_edge_x = (float)clamped_edge_x;
						
						/* Recalculate widths with strict clamped position */
						new_current_width = absolute_right_x - (int)new_edge_x;
						new_neighbor_width = ((int)new_edge_x - gap) - neighbor_tile->geom.x;
						
						/* VERIFICATION: Ensure minimum widths are respected */
						if (new_current_width < 150) new_current_width = 150;
						if (new_neighbor_width < 150) new_neighbor_width = 150;
						
						wlr_log(WLR_DEBUG, "[nixtile] HORIZONTAL STICKY MINIMUM ENFORCED: edge_x=%d, current=%d, neighbor=%d", 
					...
					wlr_log(WLR_DEBUG, "[nixtile] EDGE_LEFT: Right edge locked at %d, new left at %.1f, width=%d", 
						absolute_right_x, new_edge_x, grabc->geom.width);
					
				} else { /* EDGE_RIGHT */
					/* EDGE_RIGHT: CRITICAL - Left edge and its gap must remain completely unaffected */
					int gap = innergappx;
					
					/* Store ABSOLUTE left position - this must NEVER change */
					int absolute_left_x = grabc->geom.x;
					...
					/* Calculate what widths would be after resize */
					int new_current_width = (int)new_edge_x - absolute_left_x;
					int old_neighbor_right = neighbor_tile->geom.x + neighbor_tile->geom.width;
					int new_neighbor_width = old_neighbor_right - ((int)new_edge_x + gap);
					
					/* STICKY MINIMUM: Enforce 150px minimum with stable clamping */
					if (new_current_width < 150 || new_neighbor_width < 150) {
						/* Calculate strict boundary positions */
						int max_edge_from_current = absolute_left_x + 150;  /* Current tile limit */
						int max_edge_from_neighbor = old_neighbor_right - gap - 150;  /* Neighbor tile limit */
						...
						/* Recalculate widths with strict clamped position */
						new_current_width = (int)new_edge_x - absolute_left_x;
						new_neighbor_width = old_neighbor_right - ((int)new_edge_x + gap);
				...
				wl_list_for_each(sync_c, &clients, link) {
					...
					/* Update all tiles in current column to match new width */
					if (sync_c->column_group == grabc->column_group && sync_c != grabc) {
						sync_c->geom.x = grabc->geom.x;
						sync_c->geom.width = grabc->geom.width;
						client_set_size(sync_c, sync_c->geom.width, sync_c->geom.height);
					}
					
					/* Update all tiles in neighbor column to match new width */
					if (sync_c->column_group == neighbor_tile->column_group && sync_c != neighbor_tile) {
						sync_c->geom.x = neighbor_tile->geom.x;
						sync_c->geom.width = neighbor_tile->geom.width;
						client_set_size(sync_c, sync_c->geom.width, sync_c->geom.height);
					}
				}
				...
				wlr_log(WLR_DEBUG, "[nixtile] HORIZONTAL RESIZE: No neighbor found for edge_type=%d", resize_edge_type);
			}
		}
		...
		if (do_vertical_resize && vertical_resize_client && vertical_resize_neighbor) {
			...
			if (neighbor_tile) {
				...
				if (resize_from_top_edge) {
					...
					/* SIMPLE RULE: 150px minimum height for ALL tiles - NO EXCEPTIONS */
					int current_height = grabc->geom.height;
					int neighbor_height = neighbor_tile->geom.height;
					
					/* Calculate what heights would be after resize */
					int new_current_height = absolute_bottom_y - (int)new_edge_y;
					int new_neighbor_height = ((int)new_edge_y - gap) - neighbor_tile->geom.y;
					
					/* STICKY MINIMUM: Enforce 150px minimum with stable clamping */
					if (new_current_height < 150 || new_neighbor_height < 150) {
						/* Calculate strict boundary positions */
						int max_edge_from_current = absolute_bottom_y - 150;  /* Current tile limit */
						int max_edge_from_neighbor = neighbor_tile->geom.y + 150 + gap;  /* Neighbor tile limit */
						
						/* STICKY LOGIC: Once at minimum, stay there until user moves away from boundary */
						int clamped_edge_y;
						if (new_current_height < 150) {
							/* Current tile hitting minimum - stick to boundary */
							clamped_edge_y = max_edge_from_current;
							wlr_log(WLR_DEBUG, "[nixtile] STICKY MINIMUM: Current tile at minimum, clamping to %d", clamped_edge_y);
						} else if (new_neighbor_height < 150) {
							/* Neighbor tile hitting minimum - stick to boundary */
							clamped_edge_y = max_edge_from_neighbor;
							wlr_log(WLR_DEBUG, "[nixtile] STICKY MINIMUM: Neighbor tile at minimum, clamping to %d", clamped_edge_y);
						} else {
							/* Should not reach here, but use safer boundary */
							clamped_edge_y = (max_edge_from_current + max_edge_from_neighbor) / 2;
						}
						
						/* FORCE: Set edge to clamped position - no further movement allowed */
						new_edge_y = (float)clamped_edge_y;
						
						/* Recalculate heights with strict clamped position */
						new_current_height = absolute_bottom_y - (int)new_edge_y;
						new_neighbor_height = ((int)new_edge_y - gap) - neighbor_tile->geom.y;
						
						/* VERIFICATION: Ensure minimum heights are respected */
						if (new_current_height < 150) new_current_height = 150;
						if (new_neighbor_height < 150) new_neighbor_height = 150;
						
						wlr_log(WLR_DEBUG, "[nixtile] STICKY MINIMUM ENFORCED: edge_y=%d, current=%d, neighbor=%d", 
							(int)new_edge_y, new_current_height, new_neighbor_height);
					}
					
					/* Simple update - 150px minimum already verified above */
					...
					/* Calculate what heights would be after resize */
					int new_current_height = (int)new_edge_y - absolute_top_y;
					int old_neighbor_bottom = neighbor_tile->geom.y + neighbor_tile->geom.height;
					int new_neighbor_height = old_neighbor_bottom - ((int)new_edge_y + gap);
					
					/* STICKY MINIMUM: Enforce 150px minimum with stable clamping */
					if (new_current_height < 150 || new_neighbor_height < 150) {
						/* Calculate strict boundary positions */
						int max_edge_from_current = absolute_top_y + 150;  /* Current tile limit */
						int max_edge_from_neighbor = old_neighbor_bottom - gap - 150;  /* Neighbor tile limit */
						...
						/* Recalculate heights with strict clamped position */
						new_current_height = (int)new_edge_y - absolute_top_y;
						new_neighbor_height = old_neighbor_bottom - ((int)new_edge_y + gap);
...
resize_cleanup:
		/* Reset resize call depth at the end of successful operation */
		resize_call_depth--;
		return;
	}
...
void
build_grid_map(Monitor *m, GridTile *grid_map, int *tile_count)
{
	Client *c;
	*tile_count = 0;
	
	wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Starting build_grid_map for monitor %p", (void*)m);
	
	/* Calculate the same adjusted area as tile() function */
	...
	/* Count tiles and determine layout parameters */
	int n = 0;
	wl_list_for_each(c, &clients, link) {
		if (VISIBLEON(c, m) && !c->isfloating && !c->isfullscreen) {
			n++;
	...
	if (n == 0) {
		wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: No tiles to build grid for");
		return;
	}
	
	/* Get workspace-specific layout parameters */
	load_workspace_state();
	int optimal_columns = get_workspace_optimal_columns();
	float workspace_mfact = get_workspace_mfact();
	
	wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Layout params - tiles=%d, columns=%d, mfact=%.3f", n, optimal_columns, workspace_mfact);
	
	/* Calculate tile bounds using the same logic as tile() function */
	if (n == 1) {
		...
		if (c && *tile_count < 32) {
			...
			tile->left_edge_x = adjusted_area.x;
			tile->right_edge_x = adjusted_area.x + adjusted_area.width;
			tile->top_edge_y = adjusted_area.y;
			tile->bottom_edge_y = adjusted_area.y + adjusted_area.height;
			...
			wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Added single tile: client=%p bounds=(%d,%d)-(%d,%d)", 
		...
		wl_list_for_each(c, &clients, link) {
			...
			wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Added tile %d: client=%p column=%d bounds=(%d,%d)-(%d,%d)", 
			        *tile_count-1, (void*)c, c->column_group, 
			        tile->left_edge_x, tile->top_edge_y, tile->right_edge_x, tile->bottom_edge_y);
		...
		for (int i = 0; i < optimal_columns; i++) {
			tiles_per_column[i] = 0;
		}
		...
		wl_list_for_each(c, &clients, link) {
			...
			column_y[col] += height + innergappx;
		}
	}
	
	/* Second pass: Find neighbors and determine resizable edges */
	wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Starting neighbor detection for %d tiles", *tile_count);
		...
		for (int j = 0; j < *tile_count; j++) {
			...
			if (current->column_group != other->column_group) {
				...
				if (!(current->bottom_edge_y <= other->top_edge_y || 
					...
					/* Other tile is to the right - allow for inner gap */
					if (abs(other->left_edge_x - current->right_edge_x) <= innergappx) {
						...
						wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Horizontal neighbors: tile %d (col %d) <-> tile %d (col %d) - RIGHT edge (gap=%d)", 
						        i, current->column_group, j, other->column_group, abs(other->left_edge_x - current->right_edge_x));
					}
					/* Other tile is to the left - allow for inner gap */
					else if (abs(other->right_edge_x - current->left_edge_x) <= innergappx) {
						...
						wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Horizontal neighbors: tile %d (col %d) <-> tile %d (col %d) - LEFT edge (gap=%d)", 
						        i, current->column_group, j, other->column_group, abs(other->right_edge_x - current->left_edge_x));
					}
				}
			}
			/* Check for vertical neighbors (same column) */
				...
				if (!(current->right_edge_x <= other->left_edge_x || 
					...
					/* Other tile is below - allow for inner gap */
					if (abs(other->top_edge_y - current->bottom_edge_y) <= innergappx) {
						...
						wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Vertical neighbors: tile %d <-> tile %d (col %d) - BOTTOM edge (gap=%d)", 
						        i, j, current->column_group, abs(other->top_edge_y - current->bottom_edge_y));
					}
					/* Other tile is above - allow for inner gap */
					else if (abs(other->bottom_edge_y - current->top_edge_y) <= innergappx) {
						...
						wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Vertical neighbors: tile %d <-> tile %d (col %d) - TOP edge (gap=%d)", 
						        i, j, current->column_group, abs(other->bottom_edge_y - current->top_edge_y));
					}
				}
			}
		}
	...
	/* Debug: Print final resizable edge status for all tiles */
	wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Final resizable edge status:");
	for (int i = 0; i < *tile_count; i++) {
		GridTile *tile = &grid_map[i];
		wlr_log(WLR_ERROR, "[nixtile] GRID DEBUG: Tile %d (col %d): L=%d R=%d T=%d B=%d", 
		        i, tile->column_group, tile->left_edge_resizable, tile->right_edge_resizable, 
		        tile->top_edge_resizable, tile->bottom_edge_resizable);
	}
}
...
{
	wlr_log(WLR_ERROR, "[nixtile] *** TILERESIZE CALLED *** Starting grid-based tile resize at cursor (%.1f, %.1f)", cursor ? cursor->x : -1, cursor ? cursor->y : -1);
	
	/* CRITICAL SAFETY CHECKS */
	if (!cursor || !selmon) {
		wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: cursor=%p selmon=%p - ABORT", (void*)cursor, (void*)selmon);
		return;
	}
	
	/* Find tile under cursor */
	xytonode(cursor->x, cursor->y, NULL, &grabc, NULL, NULL, NULL);
	wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: Found client %p under cursor, isfloating=%d, isfullscreen=%d", 
		(void*)grabc, grabc ? grabc->isfloating : -1, grabc ? grabc->isfullscreen : -1);
	
	if (!grabc || grabc->isfloating || grabc->isfullscreen) {
		wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: No tiled client under cursor - ABORT");
		return;
	}
	...
	int tile_count;
	build_grid_map(selmon, grid_map, &tile_count);
	wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: Built grid map with %d tiles", tile_count);
	...
	for (int i = 0; i < tile_count; i++) {
		if (grid_map[i].tile == grabc) {
			...
			wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: Found target tile at index %d", i);
			break;
		}
	}
	
	if (!target_tile) {
		wlr_log(WLR_ERROR, "[nixtile] GRID RESIZE: Target tile not found in grid map - ABORT");
		return;
	}
...
setup(void)
{
	...
	/* STARTUP VERIFICATION: This confirms nixtile is running with new resize code */
	wlr_log(WLR_ERROR, "[nixtile] *** NIXTILE STARTUP WITH NEW RESIZE CODE - VERSION 2025-08-24 ***");
	wlr_log(WLR_ERROR, "[nixtile] *** ZONE-BASED EDGE DETECTION AND 0.1 PIXEL THRESHOLD ACTIVE ***");
...
tile(Monitor *m)
{
	...
	/* Count windows that need to be tiled - WORKSPACE-SPECIFIC VERSION */
	/* CRITICAL: Only count tiles visible in the CURRENT WORKSPACE for true isolation */
	wl_list_for_each(c, &clients, link) {
		/* WORKSPACE ISOLATION: Only count tiles visible in current workspace */
		bool has_monitor = (c->mon != NULL);
		bool not_floating = !c->isfloating;
		bool not_fullscreen = !c->isfullscreen;
		/* CRITICAL: Use VISIBLEON to ensure workspace isolation */
		bool workspace_visible = VISIBLEON(c, m);
		
		wlr_log(WLR_ERROR, "[nixtile] *** WORKSPACE TILE COUNT: Client %p - has_mon=%d, not_float=%d, not_full=%d, workspace_vis=%d, column=%d ***", 
			(void*)c, has_monitor, not_floating, not_fullscreen, workspace_visible, c->column_group);
		
		/* WORKSPACE ISOLATION: Only count if visible in current workspace */
		if (workspace_visible && not_floating && not_fullscreen) {
			n++;
		}
	}
	...
	/* WORKSPACE ISOLATION: Ensure workspace state is current before layout */
	load_workspace_state();
		...
		if (use_multi_column || force_multi_column) {
			...
			wl_list_for_each(temp_c, &clients, link) {
				...
				wl_list_for_each(count_c, &clients, link) {
					...
					/* WORKSPACE ISOLATION: Only count tiles in current workspace */
					if (VISIBLEON(count_c, m) && !count_c->isfloating && !count_c->isfullscreen) {
						workspace_tile_index++;
					}
				}
				
				/* Debug: Check tile assignment logic */
				wlr_log(WLR_ERROR, "[nixtile] *** WORKSPACE ISOLATION: Tile %p (workspace_index %d) has column_group=%d, master_tiles=%d ***", 
					(void*)temp_c, workspace_tile_index, temp_c->column_group, master_tiles);
				...
				if (temp_c->column_group < 0 || temp_c->column_group >= optimal_columns) {
					...
					/* WORKSPACE ISOLATION: Count tiles in each column (workspace-specific) */
					int column_counts[MAX_COLUMNS] = {0};
					Client *count_client;
					wl_list_for_each(count_client, &clients, link) {
						/* WORKSPACE ISOLATION: Only count tiles in current workspace */
						if (!VISIBLEON(count_client, m) || count_client->isfloating || count_client->isfullscreen)
							continue;
					...
					temp_c->column_group = target_column;
					wlr_log(WLR_ERROR, "[nixtile] *** WORKSPACE ISOLATION: Tile %p (workspace_index %d) assigned to column %d (current: %d tiles) ***", 
						(void*)temp_c, workspace_tile_index, target_column, min_tiles);
				} else {
					wlr_log(WLR_ERROR, "[nixtile] *** WORKSPACE ISOLATION: Tile %p (workspace_index %d) already has valid column_group=%d ***", 
						(void*)temp_c, workspace_tile_index, temp_c->column_group);
				}
			...
			i = 0;
			wl_list_for_each(c, &clients, link) {
				/* WORKSPACE ISOLATION: Only position tiles visible in current workspace */
				if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
					continue;
...
view(const Arg *arg)
{
	...
	/* Load complete workspace-specific state for independence */
	load_workspace_state();
	
	/* Reset workspace-specific stacking counter for "blank slate" per workspace */
	selmon->workspace_stacking_counter = 0;
	selmon->workspace_stacking_counter_initialized = false;
	wlr_log(WLR_INFO, "[nixtile] WORKSPACE SWITCH: Switched to workspace %d with independent state", get_current_workspace());
...
static int get_current_workspace() {
	...
	if (!selmon) {
		wlr_log(WLR_DEBUG, "[nixtile] STARTUP: selmon not initialized, defaulting to workspace 0");
...
static float get_workspace_mfact(void) {
	...
	if (selmon->workspace_mfact[workspace] == 0.0f) {
		selmon->workspace_mfact[workspace] = 0.5f;
		wlr_log(WLR_DEBUG, "[nixtile] WORKSPACE ISOLATION: Initialized workspace %d mfact to 0.5", workspace);
	}
	
	wlr_log(WLR_DEBUG, "[nixtile] WORKSPACE ISOLATION: Getting workspace %d mfact = %.3f", workspace, selmon->workspace_mfact[workspace]);
	return selmon->workspace_mfact[workspace];
}

/* PER-WORKSPACE STATE MANAGEMENT FUNCTIONS */
...
static void init_workspace_state(int workspace) {
	...
	if (selmon->workspace_initialized[workspace])
	...
	selmon->workspace_initialized[workspace] = true;
	wlr_log(WLR_INFO, "[nixtile] WORKSPACE ISOLATION: Initialized workspace %d with defaults and isolated pending state", workspace);
}
...
static void load_workspace_state() {
	...
	pending_right_column = selmon->workspace_pending_right_column[workspace];
	pending_left_width = selmon->workspace_pending_left_width[workspace];
	...
	/* Note: Do NOT reset resize_pending flags here as it breaks frame-synced callbacks */
	/* Only reset client pointers for safety if they're invalid */
	if (vertical_resize_client && !client_exists(vertical_resize_client)) {
		vertical_resize_client = NULL;
		vertical_resize_pending = false;
	}
	if (vertical_resize_neighbor && !client_exists(vertical_resize_neighbor)) {
		vertical_resize_neighbor = NULL;
		vertical_resize_pending = false;
	}
	if (horizontal_resize_client && !client_exists(horizontal_resize_client)) {
		horizontal_resize_client = NULL;
	}
	if (horizontal_resize_neighbor && !client_exists(horizontal_resize_neighbor)) {
...
static void save_workspace_state() {
	...
	selmon->workspace_pending_right_column[workspace] = pending_right_column;
	selmon->workspace_pending_left_width[workspace] = pending_left_width;
...
static void UNUSED set_workspace_nmaster(int value) {
	...
	selmon->nmaster = value;
	wlr_log(WLR_INFO, "[nixtile] WORKSPACE NMASTER: Set workspace %d nmaster to %d", workspace, value);
}
...
static void UNUSED set_workspace_optimal_columns(int value) {
	...
	wlr_log(WLR_INFO, "[nixtile] WORKSPACE COLUMNS: Set workspace %d columns to %d", workspace, value);
}
